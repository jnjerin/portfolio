...
const ProjectCard = ({ src, title, description, technologies, demoLink, githubLink, index }: Props) => {
  return (
    <motion.div 
      ...
      initial="hidden"
      animate="visible"
      className="relative overflow-hidden rounded-lg shadow-lg border border-[#2A0E61] bg-[#03001417] backdrop-blur-md transition-all duration-300 hover:scale-105 hover:shadow-xl"
    >
      <Image
        src={src}
        alt={title}
      ...
      <div className="p-4">
        ...
        <div className="flex justify-between">
          {demoLink && (
            <a href={demoLink} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-cyan-500 text-white rounded-full hover:bg-cyan-600 transition-colors duration-300 flex items-center justify-center">
          ...
          <a href={githubLink} target="_blank" rel="noopener noreferrer" className="px-4 py-2 bg-purple-500 text-white rounded-full hover:bg-purple-600 transition-colors duration-300 flex items-center justify-center">
